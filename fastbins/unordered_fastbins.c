...
void first_fit(size_t chunk_length){	
	...
	char *array__ = make_string(chunk_length+0x40);	
	memset(array__,0x43,chunk_length);

	char *array___ = make_string(chunk_length+0x10);	
	memset(array___,0x44,chunk_length);

	char *array____ = make_string(chunk_length+0x30);	
	memset(array____,0x44,chunk_length);

	char *array_____ = make_string(chunk_length+0x20);	
	memset(array_____,0x44,chunk_length);
	...
	memset(array__,0xFF,chunk_length+0x40);
	free_string(array__);

	memset(array___,0xFF,chunk_length+0x10);
	free_string(array___);

	memset(array____,0xFF,chunk_length+0x30);
	free_string(array____);

	memset(array_____,0xFF,chunk_length+0x20);
