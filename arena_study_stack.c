...
char *make_string(size_t length,char *source){
...
*/
void make_chunk_field(size_t chunk_length,size_t amount_of_chunks,char *source){	
	int index = 0;	
	char **chunks = malloc(amount_of_chunks*sizeof(char *)); 
	//printf("[*] chunk array head at [%p]\n",&chunks);

	for (index = 0;index < amount_of_chunks; index++){
		chunks[index] = make_string(chunk_length,source);
...
void make_chunk_field_rnd(size_t chunk_length,size_t amount_of_chunks){
	...
	for (index = 0;index < amount_of_chunks; index++){
		...
		chunks[index] = make_string(rand_size,NULL);
...
int main(int argc, char **argv){
	...
	for (index =0;index<cycles;index++){
		make_chunk_field(chunk_length,number_of_chunks,argv[4]);
